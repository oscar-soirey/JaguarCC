...
typedef void (*CVariadicFn)(int8_t *, ...);
...
static FixedC _j_struct_FixedC_default(void) { FixedC value; memset(&value, 0, sizeof(value)); return value; }
static FixedC *_j_struct_FixedC_new(void) { FixedC *value = (FixedC*)calloc(1, sizeof(FixedC)); if (!value) abort(); return value; }



void c_variadic(int8_t * fmt, ...);

void (*get_variadic(void))(int8_t *, ...);

int main(void) {
    void (*fp)(int8_t *, ...) = get_variadic();
    fp(((int8_t*)("x=%d")), 1, 2);
    return 0;
}
